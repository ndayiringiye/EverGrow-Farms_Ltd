...
const HeroSection = () => {
  ...
  return (
    <div>
      <div
        className={`relative h-screen w-full overflow-hidden mt-20 transition-colors duration-700 ${isDarkMode ? "bg-slate-950" : "bg-white"
          }`}
      >
          ...
          {slides.map((slide, index) => (
            ...
            >
              ...
              <div className="relative h-full flex items-center px-4 sm:px-6 md:px-12">
                <div
                  className={`max-w-3xl w-full backdrop-blur-md border rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 lg:p-10 shadow-2xl transition-all duration-500 ${isDarkMode
                ...
                >
                  <div className="mb-4 sm:mb-5 md:mb-6">
                    <h1
                      className={`text-2xl sm:text-3xl md:text-4xl font-extrabold leading-tight mb-2 ${isDarkMode ? "text-white" : "text-slate-900"
                        }`}
                    >
                      {slide.title}
                    </h1>
                    ...
                    {slide.subtitle && (
                      <h3
                        className={`text-xl sm:text-2xl md:text-3xl font-bold leading-tight ${isDarkMode ? "text-white" : "text-slate-800"
                          }`}
                      >
                        {slide.subtitle}
                      </h3>
                  ...
                  <p
                    className={`text-sm sm:text-base md:text-lg mb-5 md:mb-6 leading-relaxed font-light transition-colors duration-300 ${isDarkMode ? "text-white/90" : "text-slate-800/90"
                      }`}
                  >
                    {slide.description}
                  </p>
                  ...
                  <div className="flex flex-wrap gap-3 sm:gap-4">
                    ...
                    <button
                      className={`px-6 sm:px-7 md:px-8 py-2.5 md:py-3 rounded-lg font-semibold text-sm sm:text-base border transition-all duration-300 backdrop-blur-sm ${isDarkMode
        ...
        <div className="absolute bottom-8 md:bottom-12 left-1/2 -translate-x-1/2 flex gap-3 z-20">
          {slides.map((_, index) => (
            <button
              ...
              className={`h-2.5 md:h-3 rounded-full transition-all duration-300 ${index === currentSlide
        ...
        <div
          className={`absolute bottom-8 md:bottom-12 right-6 md:right-10 z-20 text-sm md:text-base font-light tracking-wider ${isDarkMode ? "text-white/80" : "text-slate-800/80"
            }`}
        >
